...
while err >= tol
    ...
    [P_inter_TAR, los_inter_TAR] = intersect_fov_sphere(pos_c2t_TAR, dcm_TAR2CAM, R, fov, nper, false);
...
if ninter == 0 || ninter == 1
    ...
    if flag_in_fov
        lon_lims = phase_angle + hs_tangency*[-1 1];
        lat_lims = hs_tangency*[-1 1];
    end    
...
elseif ninter ~= nper
    ...
    P_inter_REF(1, ixs_nan) = R*cos(hs_tangency); 
