function bins = histweight_2d(dCoords, dValues, dLimits, dGranularity, ...
                        i32Algorithm, dWindowSize, dGaussianSigma, ...
                        bSnapPointsInsideLimits)%#codegen
% HISTWEIGHT_2D Group scattered points and sum their values into fixed-size 
% 2D discrete bins with a certain weight for each point. The weight is computed
% with four different kernels evaluated on a window of given size centered on the 
% point. The possible kernels are:
%   invsquared: inverse squared distance with each vertex
%   diff: 1 minus distance normalized over maximum distance
%   area: fraction of square box that falls to each bin
%   gaussian: fraction of gaussian kernel that falls to each bin
%
% INPUTS:
...
% Find distance of each center with the center of the neighbouring bins
