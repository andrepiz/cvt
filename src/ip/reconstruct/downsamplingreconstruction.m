function img_out = downsamplingreconstruction(img_in, granularity, kernelantialiasing, filter, filter_size)
% Reconstruct an image by downsampling it with a specified reconstruction filter. 
% Before resizing, apply an antialiasing kernel, if provided.
%
...
if isempty(kernelantialiasing)
    img_conv = img_in;
else
    kern_energy_preserving = kernelantialiasing/(sum(kernelantialiasing(:)));
